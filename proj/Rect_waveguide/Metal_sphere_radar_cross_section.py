...
# 金属球の半径を設定する。unit=1e-3 なので、この値は mm 単位として扱われる。
sphere_rad = 200
# 平面波が入射する角度を x 軸基準で指定する。0 度なら +x 方向からの入射になる。
...
# openEMS の FDTD ソルバを作成する。EndCriteria は時間領域計算を打ち切る収束条件を表す。
FDTD = openEMS(EndCriteria=1e-5)

f_start =  50e6 # start frequency
f_stop  = 1000e6 # stop  frequency
f0      = 500e6
# f_start から f_stop までをカバーする広帯域ガウスパルスを励振波形として設定する。
FDTD.SetGaussExcite(  0.5*(f_start+f_stop), 0.5*(f_stop-f_start) )
# 解析領域の全 6 面に PML 吸収境界を設定し、外側境界からの反射を抑える。
